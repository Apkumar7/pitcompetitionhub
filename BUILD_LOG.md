...
## Reflection:
Honestly, when I kicked off this project, I thought there was no way I’d get something this complicated done in just a few hours. Usually, building a scouting platform with offline support, real-time sync, alliance selection, and API integrations would take a small team weeks. But the agentic workflow shrunk that timeline in a way that still feels almost unreal. Claude Code took care of all the technically necessary but soul-crushing bits—boilerplate, form fields, API wrappers, database schema. That let me focus on the stuff that actually matters: figuring out how offline sync should deal with conflicts, settling on the right alliance ranking weights, and making sure a drive team can read the tablet under pressure from three feet away. If I hadn’t had that compression, I probably would have ended up with just a login page and a schedule view. Instead, I shipped something you could actually use at a real competition.

Still, I had to jump in more than I expected. The biggest thing I had to change was the scouting form layout. Claude defaulted to a tabbed interface—it looks good in a mockup, but in a noisy arena with exhausted scouts, missing a tab and submitting incomplete data is way too easy. So I threw that out and went with a single scrolling form. Claude also messed up the alliance ranking weights at first, treating EPA and cycle speed like they mattered equally. They don’t. For playoff alliance selection, reliability and EPA are everything. That call takes real understanding of how FRC alliance selection works, not something Claude can fake.

I learned something important about myself here. I’m much better at product judgment than at systems design—and that gap is a lot wider than I wanted to admit. I had good instincts about UX and features, but when it came to infrastructure, I was nodding along to things I didn’t fully get. The clearest example: RLS policies. I read them, they seemed reasonable, so I approved them. During manual testing, I found a hole where scouts could see each other’s unsubmitted drafts. That should have jumped out at me if I’d really paid attention to the policy logic. Same thing with the real-time subscription memory leak—it was Copilot that caught it. Claude Code wrote code that looked totally fine and I just signed off without thinking about the component lifecycle. The lesson isn’t that I need to distrust Claude Code. It’s that I need to stop and really scrutinize infrastructure and security decisions, more than I do UI, because UI bugs are obvious and these ones stay hidden.

There’s also a bigger meta-skill gap to deal with. I’m good at writing prompts when I understand the feature inside out. But when I only half-get something, I can’t always spot when the output is subtly wrong. That’s probably the main thing I need to improve before my internship.

First day at my internship, I’m going to read the codebase before I touch any AI tool. The habit of writing a CLAUDE.md taught me that context matters more than anything—set it up before generating anything. I’ll keep a build log even if nobody asks, because jotting down what I changed before approving stuff is the difference between using AI as a tool and just letting it drive the car. And I’m absolutely doing the Copilot review pass on every non-trivial PR from now on. It caught things I missed, Claude missed, and it took two minutes.